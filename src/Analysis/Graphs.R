...
png("rfattempts.png")
matplot(1:10,t(my.results[c(7,8,9,12,15,20),]),type='b',col=c('purple','blue','red','cyan','green','violet'),lty=c(1,1,1,1,1),pch = 1,
        main = "RF models for retweets after an hour RMSLE vs time", ylab="RMSLE",xlab="Time after tweet creation (minutes)",ylim=c(0.55,1))
legend("bottomright",9.5,c("Alphas and log(Retweet Rates)",
                          "Just Alphas",
                          "Just log(Retweet Rates)",
                          "Alpha * log(NumFollowers)",
