        ...
        for a,ax in thedata.items():
            for tweet, table in ax.items():
                for b, rt in table.items():
                    if rt <= (dt+1)*30 and rt > dt*30:
                        for c in otheredges[b]:
                            if not c in table:
                                table[c] = -1
                                
        system('rm -rf thedata/thedata%d/alphaphiin_time%d' % (i+1,(dt+1)*30))                    
        system('mkdir thedata/thedata%d/alphaphiin_time%d' % (i+1,(dt+1)*30))
        with open('thedata/thedata%d/alphaphiin_time%d/part' % (i+1,(dt+1)*30), 'w') as f:
            for user,ax in thedata.items():
                for tweet, table in ax.items():
                    for k, v in table.items():
                        if not k == user:
                            if v <= (dt+1)*30 and v >= 0:
